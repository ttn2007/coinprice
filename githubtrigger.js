export default {
  async fetch(request, env, ctx) {
    ...
    // ===============================
    // 1️⃣ GitHub Actions Cron Trigger
    // ===============================
    if (url.pathname === "/run" && request.method === "GET") {
      ...
      console.log("🚀 GitHub Actions triggered the worker");
      ...
      const PAIRS = [
        ...
        // اگر کوین دیگه‌ای داری اینجا اضافه کن
      ];

      for (const cfg of PAIRS) {
        ctx.waitUntil(handlePair(cfg, env));
      }
      return new Response("Worker executed - check Cloudflare Logs");
    }

    // ===============================
    // 2️⃣ Telegram Webhook (رای‌گیری)
    // ===============================
...
// =========================================
// 🔥 CORE PRICE LOGIC (کاملاً مثل کد دومت)
// =========================================
async function handlePair(cfg, env) {
  console.log(`🔄 Starting handlePair for ${cfg.base}`);
  ...
  try {
    console.log(`📡 Fetching from Nobitex: ${cfg.pairIRR} & ${cfg.pairUSDT}`);
    ...
    if (dataIRR.status !== "ok" || dataUSDT.status !== "ok") {
      console.error(`❌ Nobitex status not ok for ${cfg.base}`);
      return;
    }

    currentIRR = Number(dataIRR.lastTradePrice) || 0;
    currentUSDT = Number(dataUSDT.lastTradePrice) || 0;
    console.log(`✅ Prices: IRR=${currentIRR} | USDT=${currentUSDT}`);
  } catch (err) {
    console.error(`❌ Nobitex fetch error:`, err.message);
    return;
  }

  if (currentIRR <= 0) {
    console.error(`❌ Invalid price for ${cfg.base}`);
    return;
  }
  ...
  if (now.getUTCHours() === 20 && now.getUTCMinutes() === 30 && !(await KV.get(reportKey))) {
    ...
    console.log(`📊 Sending DAILY REPORT for ${cfg.base}`);
  ...
  if (!lastUpdate || nowTs - Number(lastUpdate) > cfg.updateCooldown * 1000) {
    console.log(`📤 Sending REGULAR update for ${cfg.base}`);
    await send(API_URL, cfg.chat, cfg.thread,
      `📊 *${cfg.base}*\n\n` +
  ...
  if (alertMsg) {
    console.log(`🚨 Sending ALERT for ${cfg.base}`);
    await send(API_URL, cfg.chat, cfg.thread, alertMsg);
  }

  console.log(`✅ handlePair finished for ${cfg.base}`);
}

// =========================================
// 📤 TELEGRAM HELPERS (همه با لاگینگ کامل)
// =========================================
async function send(API_URL, chat, thread, text) {
  try {
    ...
    console.log(`📨 Attempting to send to chat ${chat} (thread: ${thread || 'main'})`);
    ...
    if (response.ok) {
      ...
      console.error(`❌ Telegram Error ${response.status}:`, result.description || result);
    }
  } catch (err) {
...
async function sendVoteMessage(API_URL, chat, thread, pair, up, down) {
  try {
    ...
    console.log(`📨 Sending NEW VOTE message for ${pair}`);
    ...
    const result = await response.json();
    if (response.ok) {
      console.log(`✅ Vote message sent! ID: ${result.result?.message_id}`);
    } else {
      console.error(`❌ Vote message error:`, result.description);
    }
  } catch (err) {
    console.error("❌ Network error sending vote message:", err.message);
  }
}
...
async function editVoteMessage(API_URL, chat, msgId, pair, up, down) {
  try {
    ...
    console.log(`✅ Vote message edited for ${pair}`);
  } catch (err) {
    console.error("❌ Error editing vote message:", err.message);
  }
}
